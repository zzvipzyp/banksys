		...
		function delSpeakerById(Obj,id,name){
		 ...
		 Confirm.show('温馨提示：', '确定要删除'+name+'么？', {
                'Delete': {
                    ...
                    'callback': function() {
                    	var param={"id":id};
                    	$.post("speakerDel.action",param,function(data){
                    		if(data=='success'){
      ...
      <tbody>
        <c:forEach items="${pageInfo.list}" var="speaker" varStatus="status">
            <tr>
	          ...
	          <td><a href="${pageContext.request.contextPath}/speakerEdit.action?id=${speaker.id}"><span class="glyphicon glyphicon glyphicon-edit" aria-hidden="true"></span></a></td>
